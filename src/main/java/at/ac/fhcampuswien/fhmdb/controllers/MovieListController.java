...
import at.ac.fhcampuswien.fhmdb.models.Sorted;
...
public class MovieListController implements Initializable {
    ...
    public void sortMovies(){
        if (sortedState == Sorted.NONE || sortedState == Sorted.DESCENDING) {
            sortMovies(Sorted.ASCENDING);
        } else if (sortedState == Sorted.ASCENDING) {
            sortMovies(Sorted.DESCENDING);
        }
    }
    // sort movies based on sortedState
    // by default sorted state is NONE
    // afterwards it switches between ascending and descending
    public void sortMovies(Sorted sortDirection) {
        if (sortDirection == Sorted.ASCENDING) {
            observableMovies.sort(Comparator.comparing(Movie::getTitle));
            sortedState = Sorted.ASCENDING;
        } else {
            observableMovies.sort(Comparator.comparing(Movie::getTitle).reversed());
            sortedState = Sorted.DESCENDING;
        }
    }
    ...
    public List<Movie> getMovies(String searchQuery, Genre genre, String releaseYear, String ratingFrom) {
        try{
            int year = releaseYear != null ? Integer.parseInt(releaseYear) : 0; // Convert releaseYear to int
            return MovieAPI.getAllMovies(searchQuery, genre.ordinal(), year, ratingFrom);
        }catch (MovieApiException e){
